...
> 一款便捷的 qPCR 数据处理工具。支持动态复孔筛选、不平衡实验设计、交互式表格输入、严格 6 步法 $2^{-\Delta\Delta Ct}$ 计算、自动统计检验，一键导出 Nature / Cell 风格图表及底层作图数据。
...
- **🔬 动态复孔 + 智能筛选** — 支持每样本 3–6 个技术重复孔；超过 3 孔时自动枚举最优组合，剔除离群废孔，只留彼此最接近的 3 孔进入计算
- **⚖️ 不平衡实验设计** — 每组生物学重复数可独立设置（2–10），完美兼容样本量不齐的实验场景（如细胞污染/动物死亡导致的数量差异）
...
- **📤 多格式导出** — 计算结果 Excel、统计检验 Excel、PNG (300+ DPI) / PDF 矢量图表 (Kaleido)、**TSV 底层作图数据**（可导入 Prism/R/Python 重绘），全部正方形 800×800 画板
- **🗂️ 合并单元格输出** — 结果表格自动合并 3+ 连续相同值，分组/样品/均值列视觉居中，淘汰孔以灰红色标注
...
页面加载后默认展示 **3 组 × 每组 3 生物学重复 × 每样本 3 技术重复 = 27 行** 的空白表格。
...
| 调整组别 | 顶部「实验分组」下拉框添加/删除组 |
| 调整技术重复数 | 「每样本技术重复孔数」选择 3–6 孔 |
| 调整生物学重复数 | 每组下方的数字框独立设置 2–10（各组可不同） |
| 修改基因名 | 直接编辑「内参基因名称」/「目的基因名称」文本框 |

### 智能复孔筛选

当技术重复孔 > 3 时，系统会在计算前自动执行离群孔剔除：

1. 枚举该样本所有孔 C(N,3) 种 3 孔组合
2. 计算每种组合的 pairwise ΔCt 差值总和
3. 选出差异最小的 3 孔（一致性最高）进入后续计算
4. 淘汰的孔在结果表格中以 ✗ 标记，灰红色背景标注

### 数据格式要求

- 每个生物学样本由所选的 **技术重复孔数** 决定行数
- Ct 值应为正数，缺失值留空
- 第一组默认作为**对照组**（校准样本）
- **各组生物学重复数可以不同**（如对照组 4 个、实验组 3 个）

### 分析流程

1. **配置参数** — 选择分组、技术重复孔数、每组生物学重复数、基因名称
2. **填入 Ct 值** — 编辑表格或从 Excel 粘贴
3. **选择检验方法** — 系统自动推荐（2 组→t-test，3+ 组→ANOVA），可手动更改
4. **点击「开始分析」** — 自动完成 6 步计算、统计检验、图表生成
5. **导出结果** — 下载 Excel 汇总表、统计检验结果、PNG/PDF 图表、TSV 作图数据
...
| 步骤 | 公式 | 粒度 |
|------|------|------|
| — | 智能筛选：N 孔 → 选 pairwise 差异最小的 3 孔 | 每样品 |
| 1 | $\Delta Ct = Ct_{target} - Ct_{ref}$；mean1 = AVG(选中 3 孔 $\Delta Ct$) | 每孔 |
| 2 | 全局基准 = AVG(所有 CONTROL 样品 mean1) | 全板 |
| 3 | $\Delta\Delta Ct = \Delta Ct$ − 全局基准 | 每孔 |
| 4 | $2^{-\Delta\Delta Ct}$；mean2 = AVG(选中 3 孔 $2^{-\Delta\Delta Ct}$) | 每孔 |
