...
def select_best_3_wells(values):
    """Select the 3 values with the smallest sum of pairwise absolute differences.

    When there are >3 technical replicate wells for a sample, this picks the
    3 wells that are most consistent with each other (least spread), effectively
    discarding outlier wells.

    Args:
        values: list of N float values, 3 ≤ N ≤ 6

    Returns:
        list of 3 indices (into the original list) of the selected wells
    """
    n = len(values)
    if n <= 3:
        return list(range(n))
    ...
    for combo in combinations(range(n), 3):
        a, b, c = values[combo[0]], values[combo[1]], values[combo[2]]
...
def compute(
    ...
    # ── Smart well selection: keep best 3 when N > 3 ──
    per_well["_well_selected"] = True
    grouped = per_well.groupby(["sample_id", "group", "target_gene"])
    for (sid, grp, tg), idxs in grouped.groups.items():
        grp_indices = list(idxs)
        if len(grp_indices) > 3:
            delta_ct_vals = per_well.loc[grp_indices, "delta_ct"].tolist()
            selected_local = select_best_3_wells(delta_ct_vals)
            selected_global = [grp_indices[i] for i in selected_local]
            discarded = [i for i in grp_indices if i not in selected_global]
            per_well.loc[discarded, "_well_selected"] = False
            per_well.loc[discarded, "delta_ct"] = np.nan

    # mean1 = AVG ΔCt per sample (only selected wells)
    mean1_map = (
        per_well[per_well["_well_selected"]]
        .groupby(["sample_id", "group", "target_gene"])["delta_ct"]
        .mean().to_dict()
    )
